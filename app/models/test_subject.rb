class TestSubject < ActiveRecord::Base
  ...
  has_many :samples, -> { order('sample_id ASC, id ASC') }, dependent: :destroy
  ...
  # Return all test subjects with the given site id
  scope :with_site, lambda { |site_id| where(site_id: site_id) }
